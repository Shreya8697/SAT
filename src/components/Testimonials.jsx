...
export default function Testimonials({ testimonials }) {
  ...
  const mobileLayout = (
    <div className="relative flex flex-col items-center min-h-[350px]">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <div className="flex">
              {[...Array(5)].map((_, i) => (
                <Star 
                  key={i}
                  className={classNames('h-3 w-3', i < testimonials[mobileIndex].rating ? 'text-amber-400 fill-current' : 'text-gray-200')} 
                />
              ))}
            </div>
          </div>
      ...
      <div className="flex justify-between items-center w-full absolute top-1/2 -translate-y-1/2 px-3 z-20">
        <button onClick={() => setMobileIndex((prev) => (prev - 1 + testimonials.length) % testimonials.length)} aria-label="Previous testimonial" className="p-2 bg-white rounded-full shadow-sm hover:bg-gray-50 transition-all hover:shadow-md">
          <ChevronLeft className="w-5 h-5 text-gray-600" />
        </button>
        <button onClick={() => setMobileIndex((prev) => (prev + 1) % testimonials.length)} aria-label="Next testimonial" className="p-2 bg-indigo-600 rounded-full shadow-sm hover:bg-indigo-700 transition-all hover:shadow-md">
      ...
      <div className="flex justify-center mt-4 space-x-2 absolute left-0 right-0 bottom-2">
        {testimonials.map((_, i) => (
          <button key={i} aria-label={`Go to testimonial ${i + 1}`} onClick={() => setMobileIndex(i)} className={`h-1.5 rounded-full transition-all duration-300 ${i === mobileIndex ? 'w-5 bg-indigo-500' : 'w-1.5 bg-gray-200'}`} />
        ))}
      </div>
    </div>
  );
  ...
  const desktopLayout = (
    <div className="relative w-full overflow-hidden">
      <div className="flex justify-center gap-4 md:gap-6 transition-all duration-500 px-2" key={desktopStart}>
        {getDesktopTestimonials().map((t) => (
          <div
            key={t.id}
            className={classNames(
              'relative transition-all duration-300 ease-in-out flex flex-col items-center rounded-xl border px-4 py-5 bg-white',
              'hover:shadow-lg',
          ...
          >
            <img src={t.image} alt={t.name} className="w-12 h-12 rounded-full object-cover ring-2 ring-indigo-100 mb-3 shadow-sm" />
            ...
            <div className="flex items-center gap-2 mt-1 mb-2">
              ...
              <div className="flex">
                {[...Array(5)].map((_, starIdx) => (
                  <Star key={starIdx} className={classNames('h-3 w-3', starIdx < t.rating ? 'text-amber-400 fill-current' : 'text-gray-200')} />
                ))}
              </div>
            </div>
        ...
        {Array.from({ length: Math.ceil(testimonials.length / 3) }).map((_, groupIdx) => (
          <button
            ...
            className={classNames(
              ...
              groupIdx * 3 === desktopStart ? 'w-6 bg-indigo-500' : 'w-2 bg-gray-200'
            )}
          />
        ))}
      </div> */}
